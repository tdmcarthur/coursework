...
combined.df <- data.frame(mget(c("y01", paste0("x", lead.zero(1:N)), 
  ...
  paste0("mode", formatC(1:nalts, flag = "0", width = max(nchar(nalts)))) )))
...
for ( i in 1:(nalts-1) )  {
  ...
  alt.params[[i]] <- paste0(alt.params[[i]], "alt", formatC(i, flag = "0", width = max(nchar(nalts))))
...
#  for (i in 1:99) {
#    ii <- formatC(i, width = max(nchar(nalts)), flag = "0")  
